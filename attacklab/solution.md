...
Level 1:
    1)  Debug program with GDB, set break point at function "getbuf", then disasamle the code, we can get:
    ...
    2) We can assumpt the return adress in right above the buffer in the stack. To verify our assumption of the buffer size and stack structure, we can run to the the return statement, 0x00000000004017bd <+21>. Then check the value of the stack pointer %rsp, we can get:
    (gdb) x $rsp
    ...
    We can see that, 0x00401976 is the where "getbuf" returns, which means our assumption.

    3) Next we should find the adress where touch1 start. We can do     this by set a break point to touch1:
                (gdb) break touch1
                Breakpoint 2 at 0x4017c0: file visible.c, line 25.
        We can see that the start adress of touch1 is: 0x4017c0

    4)  Finally, we can construct the attack string.
        ...
        For the remaining part, we should use the little-edian 64 bits adress of touch1. C will automatically add '\0' to the end of the string, which has hex representation 00. So, we only need to constuct the first 56 bits as: c0 17 40 00 00 00 00.
        Combine them, we can get our attack string: 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 c0 17 40 00 00 00 00
...
Level 2:
    1)  Analyse the problem:
        ...
        So, we can solve the problem by such procedure: 1) When we are in the function "getbuf" Use buffer overflow, change the return adress to the beginning of the buffer, where we can inject our code. 2) Then, the program start to excute our injecting code, which should change the register %rdi to the value of cook. Then push the adress of touch2() to the run-time stack and finally return. 3) Finally, the code start to excute touch2() with expression val == cookie satisified.
    ...
    3) Find the injection code
        ...
        Then by disasseble the code, we can get the binary representation of the code:
    ...
    4) Constuct attack string
        ...
        Then combine it with the last part of our attack string, which is in step 2), we can get the full attack code: 48 c7 c7 fa 97 b9 59 68 ec 17 40 00 c3 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 78 dc 61 55 00 00 00
...
Level 3:
    ...
    2)  Find the address to store the string
        ...
        3)  Write the injection code
            After finding the location of the string. We can write the injection assembly code. Fisrt, we should convert the
...
2)  First attempt
    ...
    In this case the flow of our program will be:
        1.  Using buffer overflow, store the data and code to the top of our stack
        2.  getbuf() returns. Program go to gadget
        3.  Gadget code store the cookie to %rdi. Gadget returns. Program go to touch2()
...
3)  Second attempt
    So, we need to change our method. We can cosider to pop the cookie from the stack and temporarily stored in another register. Then move it to %rdi. In this case, the flow of our program will be:
        1.  Using buffer overflow, store the data and code to the top of our stack.
        2.  getbuf() returns. Program go to gadget1, which pops cookie and stores it in a register. 
        3.  Gadget1 returns. Program go to gadget2, which move the cookie from the register in step2 to %rdi
        2.  Gadget2 returns. Program go to touch2()
    ...
    We can start by finding the "movq x, %rdi" instruction in gadget2, which has binary representation of "48 89 ??", whre ?? is different according to different x. Because it long and we need to satify it first.
...
4)  Compose the attack string
    ...
    The second part of our attack string is the address and data we put on the stack, remeber it's:
    address of gadget1 -> cookie -> address of gadget2 -> address of touch2()
    ...
    So, the whole attack sting is: 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 ab 19 40 00 00 00 00 00 fa 97 b9 59 00 00 00 00 c5 19 40 00 00 00 00 00 ec 17 40 00 00 00 00
